...
public class Emitter implements Visitor {
	...
	public Emitter (String infile, ErrorReporter reporter) {
		try {
			...
			if ( namepart.charAt(l-3) == '.' && namepart.charAt(l-2) == 'm' && namepart.charAt(l-1) == 'c') {
	...
	*/
	...
	private void emitStaticClassVariableDeclaration(Decl d) {
		...
		if (d instanceof DeclSequence) {
			...
			assert (d.isGlobal());
			Type T= typeOfDecl (D);
			emit (".field static " + D.idAST.Lexeme + " " + getTypeDescriptorLabel(T));			
		}
	}
	...
	private void emitStaticVariableReference(ID Ident, Type T, boolean write) {
		...
		ref = ref.concat(" " + ClassName + "." + Ident.Lexeme + " " + getTypeDescriptorLabel (T));
		emit (ref);
	}
	...
	private int GetNrOfFormalParams(FunDecl f) {
		int NrArgs = 0;
		Decl D = f.paramsAST;
		assert ((D instanceof EmptyFormalParamDecl) || (D instanceof FormalParamDeclSequence));
	...
	*/
	...
	public void visit(CallExpr x) {
		...
		if(isStaticMethod(F)) {
			emit(JVM.INVOKESTATIC + " lang/System/" + x.idAST.Lexeme + getDescriptor(F));
		} else {
